  ...
  return <section className={`dolo-section dolo-section-${kind} ${total !== null && formatMoney(total, false).length > 14 ? 'dolo-section-long' : ''}`} aria-label={title}>
    ...
    {lines.map((line) => {
      ...
      return <button key={line.id} type="button" onClick={() => onSelect(line.key)} className={`dolo-line ${reference ? 'dolo-line-reference' : ''} ${longAmount ? 'dolo-line-wide' : ''}`} aria-label={`View ${line.label} details`}>
        ...
        <span className={`dolo-line-amount ${line.amount_aud === null && !reference ? 'dolo-line-missing' : ''}`}>{reference ? '—' : line.amount_aud === null ? line.source_status === 'unavailable' ? 'Source blocked' : line.source_status === 'needs_review' ? 'Review source' : 'Not entered' : formatMoney(decimalToCents(line.amount_aud), false)}</span>
...
export function DoloBalanceContent({ access, onPrintMetadata }: DoloBalanceContentProps) {
  ...
  const capturedAmounts = automaticLines.filter((line) => line.amount_aud !== null).length;
  ...
  return <div className="dolo-balance"><div className="dolo-balance-inner">
    ...
    {!loading && !loadingSnapshot && !error && bundle && <>
      <div className="dolo-source-strip" role="status" aria-live="polite"><div><strong>{collecting ? 'Collecting connected sources' : selectedSnapshot?.status === 'closed' ? 'Frozen source records' : 'Connected sources'}</strong><span>{collecting ? 'Reading balances for this cut-off. Manual entries are preserved.' : `${capturedAmounts} amount${capturedAmounts === 1 ? '' : 's'} captured · ${importedSources} source-verified · ${sourceReviewCount} need source review · ${blockedSources} blocked`}</span></div><span>{latestCollection ? `Last collection: ${formatCollectedAt(latestCollection)}` : 'No collection recorded yet'}</span></div>
