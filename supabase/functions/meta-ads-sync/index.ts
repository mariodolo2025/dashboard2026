...
Deno.serve(async (req: Request) => {
  ...
  try {
    ...
    // date -> { account -> {currency, spend, conversion_value} }
    ...
    for (const acct of accounts) {
      ...
      let next: string | null = `https://graph.facebook.com/${V}/${acct}/insights?` + new URLSearchParams({
        ...
        fields: 'spend,account_currency,action_values', limit: '500',
      });
      while (next && pages < 40) {
        ...
        for (const row of (j.data || [])) {
          ...
          // single omni_purchase entry (probe matched the manual CSV to the cent);
          // last-wins is exact here. Don't sum — multiple entries would double-count.
          let conv = 0;
