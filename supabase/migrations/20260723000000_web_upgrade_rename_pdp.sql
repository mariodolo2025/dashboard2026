-- Rename the two "(PDP)" module labels to "(product page)" — the abbreviation is
-- opaque to the person reading the dashboard. Pure label change in the ev CASE;
-- nothing else in the function is touched. Full function re-stated because Postgres
...
with ev as (
  select attribution_id, action, payload, coalesce(event_timestamp, received_at)::date d,
    case
      ...
      when action like 'machine!_%' escape '!' then 'Machine finder (product page)'
      when action like 'product_recommendation%' then 'Compatible Additions (product page)'
